...
5. At-a-glance notation: the chord popover's diamond checkbox marks a full-measure hold, drawn as a diamond around the chord (Nashville style; plain-text export writes it as <C>). Each [Section] label carries a small pill for dynamics marks: Tacet, Soft, Build, Full, or a custom word with a color; the whole section gets a faint tint, a colored edge bar, and the printed name, so arrangements read at a glance even on black-and-white printouts.
