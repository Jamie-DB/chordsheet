...
5. At-a-glance notation: the chord popover's diamond checkbox marks a full-measure hold, drawn as a diamond around the chord (Nashville style, written as <C> in plain-text export). Each [Section] label carries a small pill for dynamics marks: Tacet, Soft, Build, Full, or a custom word. Every section gets a colored edge bar by type, read from its label: verse sage green, chorus orange, bridge lavender, pre-chorus gold, tag dusty rose, intro and outro slate blue, instrumental warm gray, anything else light gray. The printed page opens with one line, the title plus a key, capo, and tempo tagline, then goes straight into the chord diagrams and the chart. Artist and notes stay on screen. On the printed page (single-column layout) section titles move out of the flow into a left sidebar with the mark beneath. Tacet sections shrink to about two thirds size so they tighten up and move out of the way. The dense two-column layout keeps compact in-line labels.
...
8. Versions: once a song is finished, "New version" in the editor makes a named arrangement of it, such as "Aug 23 version". The panel orders its sections: double a chorus, add a bridge, repeat the outro x4, put a cue like "vamp while the pastor speaks" under a label, or give one pass of a chorus its own dynamics mark. Each step also has an Out toggle for sections you sit out: they shrink like Tacet and fade, and back-to-back out sections print as one run with a boxed OUT stamp, a pen-style line down through them, and a foot where you come back in. A version holds no words or chords of its own, so fixing a chord in "As written" fixes it in every version. A version's sheet is read-only, and it prints with the version name in the page footer.
